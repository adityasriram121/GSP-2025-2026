    ...
    def create_comparison_summary(self, save_path: Optional[str] = None) -> pd.DataFrame:
        ...
        comparison_data = {
            ...
            'RMSE': [
                ...
                federated_metrics.get('loss', 'N/A') if federated_metrics else 'N/A'
            ],
            'MAE': [
                centralized_metrics['mae'],
                federated_metrics.get('mae', 'N/A') if federated_metrics else 'N/A'
            ],
            'R2_Score': [
                centralized_metrics['r2'],
                federated_metrics.get('r2', 'N/A') if federated_metrics else 'N/A'
            ],
            'Bandwidth_MB': [
...
Key Findings:
 ...
 Both achieve comparable accuracy
    ...
    def plot_zone_data_from_files(self, data_dir: str = 'data', save_path: Optional[str] = None):
        """
        Create visualization of the generated zone data from CSV files.
        
        Args:
            data_dir (str): Directory containing zone CSV files
            save_path (str): Optional path to save the plot
        """
        zone_files = [
            os.path.join(data_dir, 'zone_a_rooftop_data.csv'),
            os.path.join(data_dir, 'zone_b_street_data.csv'),
            os.path.join(data_dir, 'zone_c_park_data.csv'),
            os.path.join(data_dir, 'zone_d_parking_data.csv')
        ]
        
        available_files = [f for f in zone_files if os.path.exists(f)]
        
        if not available_files:
            print("Warning: No zone data files found. Run data generation first.")
            return
        ...
        if save_path:
            plt.savefig(save_path, dpi=300, bbox_inches='tight')
            print(f"Zone data visualization saved to {save_path}")
        else:
            plt.show()
